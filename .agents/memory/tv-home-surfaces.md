...
## Studio cards use studio.imageUrl; grid-nav columns must match the layout
Studio cards (TV home row + drill-down ambient backdrop) render `studio.imageUrl` (the image set in Studio Management), falling back to a gradient from `studio.colorCode` — same field the desktop Dashboard/StudioCarousel use. TvHome shares one card "language" via `CARD_BASE` + `cardFocus()` so every surface matches.

**Gotcha:** the studio drill-down keyboard nav has a hardcoded `columns` constant for Up/Down math that MUST equal the rendered Tailwind grid (`grid-cols-N`). They drifted once (grid was 4, nav still 2) → Up/Down jumped the wrong distance. If you change the grid column count, change the nav constant in lockstep.
